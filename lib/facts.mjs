...
/**
 * Wards satisfying several conditions at once.
 *
 * This is the one question a fixed interface cannot pre-build. Five metrics
 * against five reference points with four operators is already a four-figure
 * space of combinations, and the interesting ones are the tensions — hot but
 * not classed as vulnerable, dense but green. Nobody ships a thousand buttons,
 * and a sentence expresses the query in one line.
 *
 * The model translates the sentence into conditions; the comparison itself is
 * arithmetic, so a query either matches wards or it does not.
 */
export function filterWards(conditions = []) {
  const ops = {
  ...
  for (const c of conditions.slice(0, 4)) {
    ...
    const op = ops[c?.op]
    if (!METRIC_LABELS[metric] || !op) continue
    // A reference is a named quantile or a literal value — never free text.
    let threshold, refLabel
    if (typeof c.ref === 'number' && Number.isFinite(c.ref)) {
      threshold = c.ref; refLabel = String(c.ref)
    } else if (['min', 'q1', 'median', 'q3', 'max', 'mean'].includes(c.ref)) {
      threshold = CITY[metric][c.ref]; refLabel = `city ${c.ref}`
    } else continue
    parsed.push({ metric, op: c.op, test: op, threshold, refLabel })
  }
  if (!parsed.length) return null

  const matches = WARDS
    .filter((w) => parsed.every((c) => c.test(w[c.metric], c.threshold)))
    .map((w) => ({
      ward: w.ward,
      ...Object.fromEntries(parsed.map((c) => [c.metric, w[c.metric]])),
    }))

  return {
    conditions: parsed.map((c) => ({ metric: c.metric, op: c.op, threshold: c.threshold, comparedTo: c.refLabel })),
