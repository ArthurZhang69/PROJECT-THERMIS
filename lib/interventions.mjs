...
const RULES = [
  {
    ...
    evidence: (w, osm) => {
      ...
      if (osm?.trees != null) e.push({ label: { en: 'Trees mapped in OSM', zh: 'OSM 已测绘树木' }, value: osm.trees })
      return e
    },
  ...
  },
  {
    ...
    fires: (w) => w.lst > CITY.lst.median && w.popDensity > CITY.popDensity.median,
    evidence: (w, osm) => {
      const e = [
        { label: { en: 'Land surface temp', zh: '地表温度' }, value: w.lst, rank: w.lstRank, of: CITY.wardCount, comparedTo: CITY.lst.median },
  ...
  },
  {
    ...
    fires: (w, osm) => w.lst > CITY.lst.median && w.population > CITY.population.median
      && (osm ? (osm.waterPoints ?? 0) < 3 : true),
    evidence: (w, osm) => {
      const e = [
        { label: { en: 'Population', zh: '人口' }, value: w.population, rank: w.populationRank, of: CITY.wardCount },
        { label: { en: 'Land surface temp', zh: '地表温度' }, value: w.lst, rank: w.lstRank, of: CITY.wardCount, comparedTo: CITY.lst.median },
      ]
      if (osm?.waterPoints != null) e.push({ label: { en: 'Drinking water points in OSM', zh: 'OSM 已测绘饮水点' }, value: osm.waterPoints })
      return e
    },
  ...
  },
  {
    ...
    fires: (w, osm) => w.ndvi < CITY.ndvi.q1 && w.popDensity > CITY.popDensity.median
      && (osm?.greenAreaKm2 == null || osm.greenAreaKm2 / w.areaKm2 < 0.05),
    evidence: (w, osm) => {
      const e = [
      ...
      if (osm?.greenAreaKm2 != null) {
        e.push({ label: { en: 'Green space in OSM (km²)', zh: 'OSM 绿地面积（km²）' }, value: osm.greenAreaKm2 })
        e.push({ label: { en: 'Green share of ward area (%)', zh: '绿地占本区面积（%）' }, value: pct((osm.greenAreaKm2 / w.areaKm2) * 100) })
      }
      return e
    },
...
export function interventionsFor(name, osmByCode = null) {
  ...
  const matched = RULES
    .filter((r) => r.needs.every((f) => Number.isFinite(w[f])) && r.fires(w, osm))
    .map((r) => ({
      id: r.id,
      measure: r.measure,
